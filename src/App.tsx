...
export default function App() {
  ...
  return <div className="app-shell">
    ...
    {dialog && <div className="modal-overlay" onClick={() => setDialog(null)}><section className="modal" role="dialog" aria-modal="true" aria-label={dialog === 'help' ? '使用指南' : '示例程序库'} onClick={e => e.stopPropagation()}><div className="modal-header"><div><span className="eyebrow">{dialog === 'help' ? 'A LITTLE GUIDE' : 'LEARN BY EXPLORING'}</span><h2>{dialog === 'help' ? '欢迎来到 CPU 观测站' : '选择一个好奇的起点'}</h2></div><button className="icon-button" title="关闭" onClick={() => setDialog(null)} autoFocus><X size={20} /></button></div>{dialog === 'examples' ? <div className="example-cards">{examples.map((item, index) => <button key={item.id} onClick={() => loadExample(item.id)}><span className="example-number">0{index + 1}</span><div><h3>{item.title}<span>{item.tag}</span></h3><p>{item.subtitle}</p></div><ArrowRight size={18} /></button>)}</div> : <div className="help-content">
      <p>将 C++ 代码变成看得见的数据流，理解一次运算如何发生。</p><ol><li><strong>选择架构与示例</strong> — 支持 x86、x86-64、ARM32 和 ARM64；切换架构会重置执行状态。</li><li><strong>编写与编译</strong> — 可直接编辑 main.cpp，点击编译或按 Ctrl / ⌘ + Enter。</li><li><strong>观察每一步</strong> — 运行、暂停或单步；绿色标记显示下一条指令，变化的寄存器和内存会高亮。</li><li><strong>验证结果</strong> — 原始示例包含结果断言，运行完成后可在「结果验证」查看。</li></ol>
      <h3>支持的 C++ 子集</h3><p><code>int main()</code>、int / bool 声明、固定长度 int 数组、赋值和复合赋值、后置 ++ / --、算术 / 比较 / 位运算、短路逻辑、if / else、for、while、整数 cout 输出和 return。同一函数内变量名须唯一，不同函数可使用同名变量；变量初始值默认为 0。bool 在当前教学模型中按 int 存储。</p>
      <p>支持返回 int 的普通函数定义、int / bool 按值参数、嵌套和重复调用，例如 <code>func(1, 2)</code>。参数从左到右求值，CALL / RET 单步显示调用、参数写入和返回过程；函数栈展示当前函数与调用链。函数可定义在 main 前后，非 main 函数须返回整数；暂不支持递归、函数原型、重载、void 返回类型、数组或对象参数。</p>
      <p>支持在使用前定义基础 class / struct，声明对象并读写 public 数据成员（如 <code>a.x = 1;</code>），支持成员数组、复合赋值和自增。class 默认 private，struct 默认 public；private / protected 成员禁止在普通函数中访问。对象成员按声明顺序以 4 字节单元分配，并以 a.x 等名称显示在变量和内存视图中；初始值按教学规则置零。</p>
      <h3>模型说明</h3><p>这是解释执行中间指令的教学模拟器。汇编是架构风格的伪指令映射，并非原生机器码或完整 ISA 仿真。四种模型提供不同寄存器命名、32/64 位展示和求值栈槽宽度；C++ int 数据统一为 32 位，小端序，溢出按补码截断。</p>
      <p>固定 4 字节的指令地址、状态标志、寄存器分配与数据通路均为教学简化；计数表示教学指令数，不是真实 CPU 周期。求值栈属于解释器；返回地址单独保存在调用帧，不计入 SP，各函数使用独立固定数据区，不代表原生 ABI。不包含缓存、流水线、操作系统、指针、构造函数、成员函数、继承、嵌套对象、对象数组、对象复制、类内成员初始化、STL、浮点数或完整 C++ 语义。</p>
