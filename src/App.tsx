...
function Highlight({ line }: { line: string }) {
  const parts = line.split(/(\/\/.*$|#.*$|\b(?:int|bool|for|while|if|else|return|true|false|std|cout|endl|main)\b|\b\d+\b)/g);
  return <>{parts.map((part, i) => <span key={i} className={part.startsWith('//') ? 'syntax-comment' : part.startsWith('#') ? 'syntax-include' : /^(int|bool|for|while|if|else|return|true|false)$/.test(part) ? 'syntax-keyword' : /^\d+$/.test(part) ? 'syntax-number' : /^(std|cout|endl|main)$/.test(part) ? 'syntax-function' : ''}>{part}</span>)}</>;
}
