...
函数栈视图是当前解释器的 `main` 单帧视图，以有厚度的堆叠卡片展示数据槽：绿色局部变量继续采用 `0x1000` 起的真实模拟数据地址，紫色临时求值栈从 `0x8000` 向下增长，两者分区显示。求值栈按高地址到低地址排列，标出栈底、SP 和栈顶，随执行增减层数并自动跟随栈顶。入口即展示编译期分配的全部局部数据，遵循现有平坦作用域和默认置零规则。它不代表原生 C++ 调用栈布局；自定义函数调用、递归、参数压栈及原生返回地址尚未实现，页面不会虚构这些栈帧。
