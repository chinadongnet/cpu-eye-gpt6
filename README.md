...
- 动态 CPU 示意图：指令存储器、PC、IR、控制单元、寄存器组、ALU、MAR/MDR、数据存储器和地址/数据/控制总线，支持放大查看。
- 变量、数组、小端内存字节、解释器求值栈。
...
桌面视口宽度 ≥ 1100px 且高度 ≥ 680px 时自动启用**单屏工作台**：顶部固定执行工具栏，中间为指令内存 / CPU / 函数栈，下方同屏排列源码、指令流、寄存器、控制台和变量。长内容在各面板内滚动；源码在执行时自动跟随当前行，编辑期间保留手动滚动位置。CPU 图和编辑器均可展开，按 Escape 恢复。手机及更小视口使用纵向布局，避免过度压缩内容。
...
- **左右内存结构**：左侧为 Instruction Memory，中间为 CPU，右侧默认 Stack Memory，也可切换为数据内存。
- **main 函数帧**：显示入口、返回目标、局部变量/数组/对象成员的实际数据地址、数值和字节；写入高亮并自动跟随访问位置。返回后保留最终快照。
- **临时求值栈**：展示 SP、每个槽的地址和值、栈顶、增长方向及占用字节；32 位模型每槽 4 B，64 位每槽 8 B。单步中可观察入栈和出栈。

- **指令存储器与指令集**：显示执行位置附近的指令地址和架构风格汇编，突出显示本次指令及本程序使用的指令种类。
...
函数栈视图是当前解释器的 `main` 单帧视图：局部变量继续采用 `0x1000` 起的真实模拟数据地址，临时求值栈从 `0x8000` 向下增长，两者分区显示。入口即展示编译期分配的全部局部数据，遵循现有平坦作用域和默认置零规则。它不代表原生 C++ 调用栈布局；自定义函数调用、递归、参数压栈及原生返回地址尚未实现，页面不会虚构这些栈帧。
...
支持 `A a;` / `A a{};`，多个独立对象、`int` / `bool` 数据成员及固定长度成员数组、成员读写和表达式、复合赋值及后置自增/自减。类内可写 `int x, y;`。`class` 默认 private，`struct` 默认 public，识别 public/private/protected 标签并检查 main 中的成员访问权限。
...
不支持函数调用/递归、指针、引用、构造/析构函数、成员函数、继承、嵌套对象、对象数组、对象复制、类内成员初始化、非空对象初始化列表、STL、字符串、浮点数、break/continue、前置自增、逗号表达式以及完整 C++ 类型系统。编译器对不支持语法给出诊断。
...
引擎测试涵盖六个示例在四种架构中的结果、对象实例隔离、成员布局/访问控制/高亮、状态快照、逻辑短路、表达式优先级、溢出、错误处理和死循环预算。
...
测试覆盖实际页面上的单步、连续运行、验证、架构切换、自定义编译、类成员赋值与字节布局、错误恢复、内存视图和移动端布局。
...
src/StackMemory.tsx main 教学帧、局部数据和临时求值栈
